...
/**
 * Abstract parent for "custom message attributes"
 * 
 * @author coleguy
 */
public abstract class AbstractCmaAction extends EssaAction {
	 ...
	 */
	boolean performAttributeInsert(CustomMessageAttributeDTO arg) {
		arg.setTaskId(current_task.getTaskId());
//		arg.setRuleId(new TrimString(ConstantsIf.DEFAULT_RULE));
		arg.setActive(true);
		
		arg = (CustomMessageAttributeDTO) bd.saveOrUpdate(arg);
		if (arg == null) {
	 ...
	 * @return true if success else failure
	 */
	boolean performAttributeUpdate(CustomMessageAttributeDTO arg) {
		arg = (CustomMessageAttributeDTO) bd.saveOrUpdate(arg);
		if (arg == null) {
			_log.error("saveOrUpdate returns null");
			return(false);
	...
	List<DtoIf> selectAttributes(SelectOption arg) {		
		List<DtoIf> result = bd.selectAll(new CustomMessageAttributeDTO(), arg);
		if (result == null) {
			result = new ArrayList<DtoIf>();
		}
	 ...
	 */
	CustomMessageAttributeDTO selectCurrentAttribute(Long key) {
		CustomMessageAttributeDTO dto = new CustomMessageAttributeDTO();
		dto.setRowId(key);
		dto = (CustomMessageAttributeDTO) bd.selectByRowId(dto);
		if (dto == null) {
	 ...
	 * @return true if success
	 */
	boolean selectAttributeForEdit(Long key, CmaForm form) {
		CustomMessageAttributeDTO dto = selectCurrentAttribute(key);
		if (dto != null) {
			form.setValue(dto);
			return(true);
		}
