 ...
 *   path="/user/processAdHocInsertAction"
 *   input="process.adhoc.page"
 *   scope="request" 
 *   validate="true"
 *   
 * @struts.action-forward 
 *   name="process_response" 
 *   path="process.adhoc.response.page"
 *   
 ...
 */
public final class ProcessAdHocInsertAction extends EssaAction {
	...
	public ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
		_log.debug("entry into ProcessAdHocInsertAction.execute()");

		if (isCancelled(request)) {
			_log.debug("ProcessAdHocInsertAction notes cancellation");
			return(mapping.findForward(StrutsConstantsIf.CANCEL));
		}

		if (form == null) {
			_log.error("null form in ProcessAdHocInsertAction.execute()");
...
//		final ProcessAdHocForm adhoc_form = (ProcessAdHocForm) form;
		...
		actionPrep(request);
		_log.debug("ProcessAdHocInsertAction:" + action);
		...
		if (action.equals(StrutsConstantsIf.ADHOC)) {
			...
			session.setAttribute(ConstantsIf.RESPONSE1, dto);
			*/
			return(mapping.findForward(StrutsConstantsIf.PROCESS_ADHOC_RESPONSE));
		} else {
