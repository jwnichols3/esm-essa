...
/**
 * Abstract parent for "vpo" actions
 * 
 * @author coleguy
 */
public abstract class AbstractVpoAction extends EssaAction {
	 ...
	 */
	boolean performAttributeInsert(VpoDTO arg) {
		arg.setTaskId(current_task.getTaskId());
		arg.setRuleId(current_rule);
		arg.setActive(true);
		
		arg = (VpoDTO) bd.saveOrUpdate(arg);
		if (arg == null) {
	 ...
	 * @return true if success else failure
	 */
	boolean performAttributeUpdate(VpoDTO arg) {
		arg = (VpoDTO) bd.saveOrUpdate(arg);
		if (arg == null) {
			_log.error("saveOrUpdate returns null");
			return(false);
	...
	List<DtoIf> selectAttributes(SelectOption arg) {		
		List<DtoIf> result = bd.selectAll(new VpoDTO(), arg);
		if (result == null) {
			result = new ArrayList<DtoIf>();
		}
	 ...
	 */
	VpoDTO selectCurrentAttribute(Long key) {
		VpoDTO dto = new VpoDTO();
		dto.setRowId(key);
		dto = (VpoDTO) bd.selectByRowId(dto);
		if (dto == null) {
	 ...
	 * @return true if success
	 */
	boolean selectAttributeForEdit(Long key, VpoForm form) {
		VpoDTO dto = selectCurrentAttribute(key);
		if (dto != null) {
			form.setValue(dto);
			return(true);
		}
