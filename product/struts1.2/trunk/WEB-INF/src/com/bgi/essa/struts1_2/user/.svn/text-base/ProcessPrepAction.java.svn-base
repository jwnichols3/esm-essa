...
/**
 * Prepare to service "process" rule operations
 * 
 * @struts.action 
 *   name="processForm" 
 *   path="/user/processPrepAction"
 *   scope="request" 
 *   validate="false"
 *   
 * @struts.action-forward 
 *   name="audit_rule" 
 *   path="process.audit.page"
 *   
 * @struts.action-forward 
 *   name="browse_rule" 
 ...
 */
public final class ProcessPrepAction extends AbstractProcessAction {
	...
	public ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
		_log.debug("entry into ProcessPrepAction.execute()");
		...
		try {
			...
			_log.error("ProcessPrepAction", exception);
		}

		_log.debug("ProcessPrepAction forward:" + forward);
