...
public class SimpleI18NBeanBeanInfo extends SimpleBeanInfo
{
   ...
   public SimpleI18NBeanBeanInfo() throws java.beans.IntrospectionException
   {
	  ...
	  bd.setValue("literal",res.getString("attribute.bean.literal"));
   ...
   public PropertyDescriptor[] getPropertyDescriptors() 
   {
      ...
      try
      {
         PropertyDescriptor descriptor = new PropertyDescriptor("flag", test.javabean.SimpleBean.class, "isFlag", "setFlag");
      ...
      try
      {
         PropertyDescriptor descriptor = new PropertyDescriptor("name", test.javabean.SimpleBean.class, "getName", "setName");
      ...
      try
      {
         ...
         PropertyDescriptor descriptor = new PropertyDescriptor("count", test.javabean.SimpleBean.class, "getCount", null);
      ...
      try
      {
         PropertyDescriptor descriptor = new PropertyDescriptor("parent", test.javabean.SimpleBean.class, "getParent", null);
   ...
   public MethodDescriptor[] getMethodDescriptors() {
      ...
      if (method != null) {
         final String methodName = "myFirstMethod";
         ...
         descriptor.setDisplayName(res.getString("method."+ methodName +".displayName"));
         descriptor.setDisplayName(res.getString("method."+ methodName +".shortDescription"));
      ...
      for (int j = 0; j != m.length; j++) {
         if (m[j].getName().equals("mySecondMethod")) {
			...
			if (!params[paramNum++].equals(java.lang.String.class))
				continue;
			if (params.length <= paramNum)
				continue; // too few parameters
			if (!params[paramNum++].equals(boolean.class))
				continue;
			if (params.length != paramNum)
				continue; // to much parameter
            method = m[j];
      ...
      if (method != null) {
         final String methodName = "mySecondMethod";
         final ParameterDescriptor[] pd = new ParameterDescriptor[method.getParameterTypes().length];
         int pidx = 0;
         pd[pidx] = new ParameterDescriptor();
         pd[pidx].setName("param1");
         pd[pidx].setDisplayName(res.getString("method." + methodName + '.' + pd[pidx].getName() + ".displayName"));
