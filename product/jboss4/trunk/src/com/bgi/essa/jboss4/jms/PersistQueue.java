...
public class PersistQueue {
	...
	/**
	 * Write the specified transfer object to the persistence queue (database insert).
	 * 
	 * @param arg DTO to persist
	 * @return true if success else failure
	 */
	public boolean queueWriter(DtoIf arg) {
		try {
			Context context = new InitialContext();
			Queue queue = (Queue) context.lookup(ConstantsIf.PERSIST_QUEUE);
	 ...
	 * Logger
	 */
	private final Log _log = LogFactory.getLog(PersistQueue.class);
}
