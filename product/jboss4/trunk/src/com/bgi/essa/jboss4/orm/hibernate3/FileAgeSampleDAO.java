...
/**
 * FileAgeSample data access operations 
 * 
 * @author coleguy
 */
public class FileAgeSampleDAO extends AbstractDao {

	/**
	 * Select a specific file age sample row.
	 * 
	 * @param arg selection key, maps to FileAgeSample.id
	 * @return selected item or null if not found
	 */
	@Override
	public FileAgeSampleDTO selectRowById(final Long arg) {
		_log.debug("select fileage sample by id:" + arg);

		final String query_string = "from FileAgeSampleDTO dto where dto.id = :arg1";

		return((FileAgeSampleDTO) getRow(new TrimString(arg.toString()), query_string));
	}
	...
	public List<DtoIf> selectRowByKey(final DtoIf dto, final SelectOption option) {
		...
		FileAgeSampleDTO temp = (FileAgeSampleDTO) dto;
		
		String query_string = null;

		if (option.equals(SelectOption.SELECT_ACTIVE)) {
			query_string = "from FileAgeSampleDTO dto where dto.fauxActive = 'T' and dto.ruleId = :arg1";
		} else if (option.equals(SelectOption.SELECT_ALL)) {
			query_string = "from FileAgeSampleDTO dto where dto.ruleId = :arg1";
		} else if (option.equals(SelectOption.SELECT_INACTIVE)) {
			query_string = "from FileAgeSampleDTO dto where dto.fauxActive = 'F' and dto.ruleId = :arg1";
		} else {
	...
	public List<DtoIf> selectAllRows(final SelectOption option) {
		...
		if (option.equals(SelectOption.SELECT_ACTIVE)) {
			query_string = "from FileAgeSampleDTO dto where dto.fauxActive = 'T'";
		} else if (option.equals(SelectOption.SELECT_ALL)) {
			query_string = "from FileAgeSampleDTO dto";
		} else if (option.equals(SelectOption.SELECT_INACTIVE)) {
			query_string = "from FileAgeSampleDTO dto where dto.fauxActive = 'F'";
		} else {
			_log.error("unknown selection option:" + option);
			return(null);
		}
