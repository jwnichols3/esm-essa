 ...
 * @author coleguy
 */
public class UpTimeSampleDtoTest extends TestCase {

	/**
	 * @param name
	 */
	public UpTimeSampleDtoTest(final String name) {
		super(name);
	}
	...
	public void test02() {
		...
		// boot time tests
		//
		final ZuluTime zt2 = new ZuluTime();
		assertNotNull("test02e", zt2);
		
		dto1.setBootTime(zt2);
		assertTrue("test02f", dto1.getBootTime().equals(zt2));
		
		exception_flag = false;
		try {
			dto1.setBootTime((ZuluTime) null);
		} catch(final NullPointerException exception) {
			exception_flag = true;
		}
		assertTrue("test02g", exception_flag);
	}

	/**
	 * Logger
	 */
	private final Log _log = LogFactory.getLog(UpTimeSampleDtoTest.class);
}
